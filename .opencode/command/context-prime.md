---
description: Load essential project context for a new agent session by reading context documentation
---
...
- If directory doesn't exist or is empty:
  ...
  - Exit gracefully
- Count available context files: `ls -1 .opencode/context/*.md 2>/dev/null | wc -l`
- Report: "📁 Found {count} context files to load"

### 2. File Integrity Check
- For each context file found:
  - Verify file is readable: `test -r ".opencode/context/{file}" && echo "readable"`
  - Check file has content: `test -s ".opencode/context/{file}" && echo "has content"`
