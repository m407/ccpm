---
description: Sync local development progress to GitLab as issue comments for transparent audit trail and status updates
---
...
3. **Local Updates Check:**
   - Check if `.opencode/epics/*/updates/$ARGUMENTS/` directory exists
...
- Read from `.opencode/epics/{epic_name}/updates/$ARGUMENTS/`
...
```bash
glab issue comment #$ARGUMENTS --body-file {temp_comment_file}
```
