---
description: Parse a Product Requirements Document and generate a technical implementation epic with task breakdown
---
...
2. **Verify PRD exists:**
   - Check if `.opencode/prds/$ARGUMENTS.md` exists
...
3. **Validate PRD frontmatter:**
   ...
   - If frontmatter is invalid or missing, tell user: "❌ Invalid PRD frontmatter. Please check: .opencode/prds/$ARGUMENTS.md"
   - Show what's missing or invalid

4. **Check for existing epic:**
   - Check if `.opencode/epics/$ARGUMENTS/epic.md` already exists
...
5. **Verify directory permissions:**
   - Ensure `.opencode/epics/` directory exists or can be created
...
### 1. Read the PRD
- Load the PRD from `.opencode/prds/$ARGUMENTS.md`
...
Create the epic file at: `.opencode/epics/$ARGUMENTS/epic.md` with this exact structure:
...
progress: 0%
prd: .opencode/prds/$ARGUMENTS.md
...
- `.opencode/epics/$ARGUMENTS/` (directory)
- `.opencode/epics/$ARGUMENTS/epic.md` (epic file)
...
1. Confirm: "✅ Epic created: .opencode/epics/$ARGUMENTS/epic.md"
2. Show summary of:
